...
Version 0.47.0 is a **minor release** closing a long VT-fidelity and security-audit backlog: color-palette stack (XTPUSHCOLORS/XTPOPCOLORS), legacy alt-screen modes 47/1047/1048, X10 mouse tracking, DECSACE, `OSC 1337;CurrentDir=`, `Terminal.diff_snapshots()`, real Python type stubs, a criterion benchmark suite, and streaming/asciicast fixes. **Breaking for Rust `rlib` consumers only:** MSRV is now 1.98 and some dead APIs were removed; Python users are unaffected. See [What's New in 0.47.0](#whats-new-in-0470) below, and [CHANGELOG.md](CHANGELOG.md) for complete release notes.
