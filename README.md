...
Version 0.49.0 is a **minor release** completing graphics fidelity and recording interoperability: recordings export in asciicast v3 format with per-graphic `g` events carrying pixels and geometry, parameterized XTPUSHCOLORS/XTPOPCOLORS slot forms (`CSI Pi # P/Q`) store and restore palette snapshots in place, DECSDM (`CSI ? 80 h/l`) gives sixel graphics home-position display placement, Kitty placements advance the cursor below the image (`C=1` suppresses), and the web terminal renders Sixel and iTerm2 inline images. No breaking changes. See [What's New in 0.49.0](#whats-new-in-0490) below, and [CHANGELOG.md](CHANGELOG.md) for complete release notes.
