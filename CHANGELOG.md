...
- **A stalled par-mux control client is evicted instead of buffering forever (Rust `mux` feature)** (`src/mux/server.rs`; ARC-011). Per-client broadcast queues are bounded at 4096 lines (~16 MiB worst case); a client whose queue fills is disconnected cleanly, tmux's policy for a control client that stops draining, and a draining sibling provably loses nothing.
