...
- **Build tooling**: the Makefile `lint`/`typecheck`/`clippy` targets and the pre-commit clippy hook no longer pass `--all-features` (which selects the invalid `sim`+`python` pair now rejected by a `compile_error!` guard).
