...
/// Emit the screenshot surface for `$ty` (QA-004).
///
/// `screenshot`/`screenshot_to_file`/`screenshot_config`/
/// `screenshot_to_file_config` were duplicated verbatim across
/// `PyTerminal` and `PyPtyTerminal`; both now share this macro, which
/// reaches the terminal through [`TerminalAccess::term_ref`] — the same
    ...
    ($ty:ty) => {
        ...
        impl $ty {
            ...
            fn screenshot(
                ...
                // Theme settings from the terminal are the defaults when the
                // corresponding args are not explicitly provided.
                let term_bold_brightening = t.bold_brightening();
                let term_bg_color = t.default_bg().to_rgb();
                ...
                let config = ScreenshotConfig {
                    ...
                    sixel_render_mode: $crate::python_bindings::conversions::parse_sixel_mode(
                        sixel_mode,
                    )?,
                    link_color,
                    bold_color,
            ...
            fn screenshot_config(
                &self,
                config: &$crate::python_bindings::screenshot_config::PyScreenshotConfig,
                scrollback_offset: usize,
            ) -> pyo3::PyResult<Vec<u8>> {
