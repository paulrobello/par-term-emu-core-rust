...
fn test_dirty_region_tracking() {
    ...
    assert_eq!(term.get_dirty_region(), Some((0, 0, 0, 79)));
...
fn test_bell_events() {
    ...
    assert!(matches!(
        events[0],
        BellEvent::WarningBell(_) | BellEvent::VisualBell
    ));
...
fn test_mode_introspection() {
    ...
    assert_eq!(term.left_right_margins(), (0, 79));
    ...
    let (left, right) = term.left_right_margins();
...
fn test_bulk_read_operations() {
    ...
    // Get visible region
    let visible = term.get_rectangle(0, 0, 4, 9);
...
fn test_rectangle_operations() {
    ...
    let visible = term.get_rectangle(0, 0, 9, 19);
...
fn test_terminal_events() {
    ...
    assert!(events
        .iter()
        .any(|e| matches!(e, TerminalEvent::BellRang(_))));
...
fn test_resize_records_event_when_recording() {
    ...
    assert!(session.events.iter().any(|event| {
        matches!(event.event_type, RecordingEventType::Resize) && event.metadata == Some((90, 30))
    }));
}
...
fn test_clipboard_sync_event_limits() {
    ...
    let middle = &events[0].content;
    assert!(middle.contains("[truncated]"));
    assert_eq!(events[1].content, "final");
}
...
fn test_search_multiple_matches_same_line() {
    ...
    let matches = term.search("test", options).unwrap();
...
fn test_search_unicode_byte_offset() {
    ...
    let matches = term.search("World", options).unwrap();
    assert_eq!(matches.len(), 1);
...
fn test_search_unicode_query() {
    ...
    let matches = term.search("こんにちは", options).unwrap();
    assert_eq!(matches.len(), 1);
...
fn test_search_unicode_multiple_matches() {
    ...
    let matches = term.search("日本", options).unwrap();
    assert_eq!(matches.len(), 2);
...
fn test_detect_file_paths() {
    ...
    assert!(
        has_path || items.is_empty(),
        "File path detection may vary by implementation"
    );
}
...
fn test_detect_semantic_items_git_hash() {
    ...
    let has_git_hash = items
        .iter()
        .any(|item| matches!(item, DetectedItem::GitHash(hash, _, _) if hash.contains("a1b2c3")));
...
fn test_detect_semantic_items_ip_address() {
    ...
    let has_ip = items
        .iter()
        .any(|item| matches!(item, DetectedItem::IpAddress(ip, _, _) if ip == "192.168.1.100"));
    assert!(has_ip);
}
...
fn test_detect_semantic_items_email() {
    ...
    let has_email = items.iter().any(
        |item| matches!(item, DetectedItem::Email(email, _, _) if email == "user@example.com"),
    );
    assert!(has_email);
}
...
fn test_tmux_control_mode_suppresses_raw_protocol() {
    ...
    assert!(
        notifications.len() >= 2,
        "Should have at least Begin and Output notifications"
    );
